...
- Add a custom method that's not defined in the trait definition it's implementation block (e.g. add a method named `another` to the `impl` block starting with line `impl Flip for Flipper {` in the [trait-flipper contract](https://github.com/ink-analyzer/ink-vscode/blob/master/test-fixtures/trait-flipper/lib.rs) in the test-fixtures directory).
